...
function App() {
  ...
  return <main className="game-page">
    <section className={`game-console ${phase === 'transition' ? 'is-transitioning' : ''}`}>
      <aside className="controls-panel"><div className="control-group"><p>Move</p><div className="d-pad"><span/><button onClick={() => move(0, -1)}>W</button><span/><button onClick={() => move(-1, 0)}>A</button><button onClick={() => move(0, 1)}>S</button><button onClick={() => move(1, 0)}>D</button></div><small>WASD or arrow keys</small><button className="menu-key" onClick={() => setMenuOpen(true)}>E <span>Menu</span></button><button className="sound-key" onClick={toggleMute}>{started && !muted ? '♪ On' : '♪ Off'}</button></div></aside>
